...
public class Square {
	...
	public boolean canMove(Direction direction) {
		...
		boolean rotate = true;
            ...
            if (col == 0 || grid.isSet(row, col - 1))
                rotate = false;
        if (!(piece instanceof BarShape) ) {
            if (col == (Grid.WIDTH - 1) || grid.isSet(row, col + 1))
                            rotate = false;
            if (row == (Grid.HEIGHT - 1) || grid.isSet(row + 1, col))
                            rotate = false;
        }
        else if (piece instanceof BarShape ) {
            if (col == (Grid.WIDTH - 2) || grid.isSet(row, col + 1))
                            rotate = false;
            if (row == (Grid.HEIGHT - 1) || grid.isSet(row + 1, col))
                            rotate = false;
        }
	...
	public void draw(Graphics g) {
        ...
        public void rotate(int rCenter, int cCenter){
            int rEnd, cEnd;
            rEnd = rCenter + (col - cCenter);
            cEnd = cCenter + (rCenter - row);
            row = rEnd;
            col = cEnd;
        }
}